...
client.on('ready', async () => {
  ...
  const commands = [
    ...
    { name: 'volumeup', description: 'Increase volume by 10%' },
    { name: 'volumedown', description: 'Decrease volume by 10%' },
    ...
    { name: 'ai', description: 'Tanya bot AI', options: [{ name: 'prompt', description: 'Pertanyaan kamu', type: 3, required: true }] }
  ];
...
async function resolveTracks(query, requester) {
  ...
  if (isUrl) {
    try {
      return await riffy.resolve({ query, requester });
    } catch (error) {
  ...
  for (const q of attempts) {
    try {
      const res = await riffy.resolve({ query: q, requester });
      if (res && Array.isArray(res.tracks) && res.tracks.length) {
        return res;
      }
      if (!lastResult) lastResult = res;
    } catch (error) {
      console.error('Resolve search error:', q, error);
    }
  }
...
function pickBestTrack(query, tracks) {
  ...
  for (const t of tracks) {
    const title = normalizeText(t.info?.title || '');
    const author = normalizeText(t.info?.author || '');
    if (!title) continue;
    ...
    if (title === q) {
      ...
      for (const word of qWords) {
        if (!word) continue;
        if (title.includes(word)) common += 2;
        if (author.includes(word)) common += 3;
      }
      score = common;
    }

    if (author.includes('al yete')) {
      score += 20;
    }
...
function createNowPlayingContainer(player, track, disabled = false) {
  ...
  const container = new ContainerBuilder()
    ...
    .addTextDisplayComponents(
      new TextDisplayBuilder()
        .setContent(`**Duration:** ${formatTime(info.length || 0)} • **Requested By:** <@${track.info.requester}> • **Volume:** ${volume}%`)
    )
...
client.on('messageCreate', async (message) => {
  ...
  const hasPrefix = config.enablePrefix && lower.startsWith(config.prefix.toLowerCase());

  if (hasPrefix) {
    const body = lower.slice(config.prefix.length).trim();
    const [rawCmd] = body.split(/ +/);
    let cmd = rawCmd;
  ...
  const callHadi = tokens.some((t) => hadiTriggers.includes(t));
  if (callHadi) {
    await message.channel.send('nrhd dan cia orang paling ganteng dan cantik').catch(() => {});
  }
  ...
  if (isToxic) {
    ...
    await message.channel
      .send(`UDAH TUA TOXIC MULU BAU TANAH JUGA MIKIR!!! <@${message.author.id}>`)
      .catch(() => {});
    return;
  }
  ...
  const requiredRoleId = config.chatRoleId;
  if (requiredRoleId && (!message.member || !message.member.roles.cache.has(requiredRoleId))) {
    return;
  }

  const allowedChannelId = config.aiChannelId;
  ...
  try {
    ...
    const response = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      headers: {
        ...
        'X-Title': client.user.username || 'Discord Bot'
      },
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: 'Kamu adalah asisten Discord yang selalu menjawab dalam bahasa Indonesia yang santai tapi sopan. Jangan gunakan bahasa lain kecuali diminta jelas oleh user.'
          },
          {
            role: 'user',
            content
    ...
    if (!response.ok) {
      ...
      console.error('OpenRouter API error (auto message):', response.status, text);
      if (response.status === 429 || text.includes('rate-limited')) {
        await message.channel.send('Fitur AI lagi limit (429), coba lagi sebentar lagi ya.').catch(() => {});
      }
      return;
    }
  ...
  const trimmed = answer.length > 1800 ? `${answer.slice(0, 1800)}...` : answer;

  const description = `**Pertanyaan:**\n${content}\n\n**Jawaban:**\n${trimmed}`;
  const container = createSimpleContainer('AI NRHDBOT', description, config.emojis.info);
...
client.on('interactionCreate', async (interaction) => {
  if (interaction.isButton()) {
    ...
    switch (interaction.customId) {
      ...
      case 'volumedown': {
        const message = nowPlayingMessages.get(player.guildId);
        const currentVolume = typeof player.volume === 'number' ? player.volume : 100;
        const newVolume = Math.max(1, currentVolume - 10);
        player.setVolume(newVolume);
        if (message && player.current) {
          const updatedContainer = createNowPlayingContainer(player, player.current);
          await message.edit({ components: [updatedContainer], flags: MessageFlags.IsPersistent | MessageFlags.IsComponentsV2 }).catch(() => {});
  ...
  if (commandName === 'play') {
    ...
    try {
      ...
      const resolve = await resolveTracks(query, member.user.id);
      ...
      if (resolve.loadType === 'playlist') {
        ...
        const track = pickBestTrack(query, resolve.tracks) || resolve.tracks[0];
        track.info.requester = member.user.id;
        player.queue.add(track);
  ...
  if (commandName === 'ai') {
    ...
    if (requiredRoleId && (!member || !member.roles.cache.has(requiredRoleId))) {
      return interaction.reply({ content: `${config.emojis.error} Kamu tidak punya izin untuk memakai fitur AI ini`, ephemeral: true });
    }

    const apiKey = process.env.OPENROUTER_API_KEY;
    if (!apiKey) {
      return interaction.reply({ content: `${config.emojis.error} Fitur AI belum dikonfigurasi (API key belum di-set)`, ephemeral: true });
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 429 || text.includes('rate-limited')) {
          return interaction.editReply({ content: `${config.emojis.error} Fitur AI lagi limit (429), coba lagi sebentar lagi ya.` });
        }
        return interaction.editReply({ content: `${config.emojis.error} Gagal menghubungi AI` });
      }
      ...
      await interaction.editReply({ content: `${config.emojis.error} Terjadi error saat memanggil AI` });
    }
  }
  ...
  if (commandName === 'volumeup') {
    ...
    const newVolume = Math.min(100, currentVolume + 10);
    player.setVolume(newVolume);
    const container = createSimpleContainer('Volume Increased', `Volume: ${currentVolume}% → ${newVolume}%`, config.emojis.volume);
    await interaction.reply({ components: [container], flags: MessageFlags.IsComponentsV2 });
  }

  if (commandName === 'volumedown') {
    const player = riffy.players.get(guild.id);
    if (!player) return interaction.reply({ content: `${config.emojis.error} No player found`, ephemeral: true });
    if (!member.voice.channel || member.voice.channel.id !== player.voiceChannel) {
      return interaction.reply({ content: `${config.emojis.error} You need to be in the same voice channel`, ephemeral: true });
    ...
    const newVolume = Math.max(1, currentVolume - 10);
    player.setVolume(newVolume);
    const container = createSimpleContainer('Volume Decreased', `Volume: ${currentVolume}% → ${newVolume}%`, config.emojis.volume);
    await interaction.reply({ components: [container], flags: MessageFlags.IsComponentsV2 });
  }
        ...
        client.on('messageCreate', async (message) => {
          ...
          if (command === 'play') {
            ...
            try {
              ...
              const resolve = await resolveTracks(query, message.author.id);
          ...
          if (command === 'ai') {
            const requiredRoleId = config.chatRoleId;
            if (requiredRoleId && (!message.member || !message.member.roles.cache.has(requiredRoleId))) {
              return message.reply(`${config.emojis.error} Kamu tidak punya izin untuk memakai fitur AI ini`);
            }

            const apiKey = process.env.OPENROUTER_API_KEY;
            if (!apiKey) {
              return message.reply(`${config.emojis.error} Fitur AI belum dikonfigurasi (API key belum di-set)`);
            }

            const prompt = args.join(' ');
            if (!prompt) {
              return message.reply(`${config.emojis.error} Tolong tulis pertanyaan setelah command, contoh: \`${config.prefix}ai apa kabar?\``);
            }
            ...
            try {
              ...
              if (!response.ok) {
                const text = await response.text();
                console.error('OpenRouter API error (prefix):', response.status, text);
                if (response.status === 429 || text.includes('rate-limited')) {
                  return message.reply(`${config.emojis.error} Fitur AI lagi limit (429), coba lagi sebentar lagi ya.`);
                }
                return message.reply(`${config.emojis.error} Gagal menghubungi AI`);
              }
              ...
              const description = `**Pertanyaan:**\n${prompt}\n\n**Jawaban:**\n${trimmed}`;
              const container = createSimpleContainer('AI Response', description, config.emojis.info);

              await message.reply({ components: [container], flags: MessageFlags.IsComponentsV2 });
            } catch (error) {
              console.error('AI prefix command error:', error);
              await message.reply(`${config.emojis.error} Terjadi error saat memanggil AI`);
            }
          }
